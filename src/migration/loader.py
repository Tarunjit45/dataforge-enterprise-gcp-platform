...
class DataLoader:
    ...
    def load_batch(
        ...
        if not records:
            logger.info(
                f"No records provided to load for '{target_schema}.{table_name}'. Skipping."
            )
        ...
        if expected_checksum:
            ...
            if not self.checksum_engine.compare_checksums(expected_checksum, batch_checksum):
                raise PipelineError(
                    f"Checksum mismatch for table batch '{table_name}' prior to loading!"
                )
        ...
        while retry_count <= self.max_retries:
            try:
                logger.info(
                    f"Loading batch of {len(records)} records into AlloyDB table '{target_schema}.{table_name}'..."
                )
                ...
                logger.info(
                    f"Successfully loaded batch for '{table_name}'. Cumulative loaded rows: {checkpoint.rows_loaded}."
                )
                return checkpoint
                ...
                retry_count += 1
                logger.warning(
                    f"Error loading table '{table_name}' (Attempt {retry_count}/{self.max_retries}): {e}"
                )
                if retry_count > self.max_retries:
                    raise PipelineError(
                        f"Failed to load table '{table_name}' after {self.max_retries} retries: {e}"
                    ) from e
                time.sleep(1)
